...
describe("QuestieTDBMock", function()
    ...
    describe("RequireContract", function()
        it("accepts Contract Version 2", function()
            local ok, message = LibQuestieDB.RequireContract(2)
    ...
    describe("entity reads", function()
        ...
        it("returns a fresh copy of a table value on every read", function()
            local spawns = LibQuestieDB.Npc.Get(30, "spawns")
            spawns[12] = nil

            assert.are_same({[12] = {{36.43, 55.89}}}, LibQuestieDB.Npc.Get(30, "spawns"))
            assert.are_not_equal(LibQuestieDB.Npc.Get(30, "spawns"), LibQuestieDB.Npc.Get(30, "spawns"))
            assert.are_not_equal(LibQuestieDB.Npc.GetRaw(30, "spawns"), LibQuestieDB.Npc.GetRaw(30, "spawns"))
        end)
    ...
    describe("translation slots", function()
        it("keeps translation owner precedence through replacement, locale switches, and withdrawal", function()
            mock.SetBaseRow("Npc", 30, {[npcKeys.name] = "Spider", [npcKeys.zoneID] = 12})
            LibQuestieDB.Corrections.Set("Policy", "Npc", "normal", {[30] = {[npcKeys.name] = "Corrected", [npcKeys.zoneID] = 99}})
            local rows = {[30] = {[npcKeys.name] = "First"}}
            LibQuestieDB.l10n.SetCorrection("FirstOwner", "ukUA", "Npc", "names", rows)
            rows[30][npcKeys.name] = "Mutated"
            assert.are_same("Corrected", LibQuestieDB.Npc.name(30))
            LibQuestieDB.l10n.SetLocale("ukUA")
            assert.are_same("First", LibQuestieDB.Npc.name(30))
            LibQuestieDB.l10n.SetCorrection("SecondOwner", "ukUA", "Npc", "names", {[30] = {[npcKeys.name] = "Second"}})
            LibQuestieDB.l10n.SetCorrection("FirstOwner", "ukUA", "Npc", "names", {[30] = {[npcKeys.name] = "Replacement"}})
            assert.are_same("Second", LibQuestieDB.Npc.name(30))
            assert.are_same("SecondOwner", LibQuestieDB.GetProvenance("Npc", 30, "name"))
            assert.are_same(99, LibQuestieDB.Npc.zoneID(30))
            LibQuestieDB.l10n.SetCorrection("SecondOwner", "ukUA", "Npc", "names", nil)
            assert.are_same("Replacement", LibQuestieDB.Npc.name(30))
            LibQuestieDB.l10n.SetLocale("enUS")
            assert.are_same("Corrected", LibQuestieDB.Npc.name(30))
            assert.are_same("Policy", LibQuestieDB.GetProvenance("Npc", 30, "name"))
        end)
    end)
