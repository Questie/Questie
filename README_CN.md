...
[![Downloads Latest](https://img.shields.io/github/downloads/Questie/Questie/v11.24.1/total.svg)](https://github.com/Questie/Questie/releases/latest)
...
将 `<expansion>` 替换为你想要验证的扩展包名称 (可查看 `cli` 文件夹获取可用脚本).
...
### 按区域划分的任务
- Questie 会列出某个区域内已完成和可接取的所有任务，一项都别错过。 (左键点击小地图按钮 (或者输入 `/questie journey`) 然后选择 “按区域划分的任务” 标签)
