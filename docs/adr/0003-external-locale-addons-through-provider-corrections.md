...
`questLookup`, `npcNameLookup`, `objectLookup`). QuestieDB has no generated Base block for such
...
During Login Initialization, Questie requires QuestieDB Contract Version 2 and checks that
...
The publisher converts each lookup to rows keyed by QuestieDB's numeric entity field indexes. It
...
The external locale does not have to be active when published. QuestieDB stores the locale-specific
...
it was a pass-through around one publisher. Moving the adapter into QuestieDB would make the
