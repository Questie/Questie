...
During the Source-mode tests, only beta's provider link was repointed to `\\wsl.localhost\Ubuntu\home\logon\projects\QuestieDB`. At David's request, that change was undone by moving the preserved original symlink back. Beta again points through `C:\Games\World of Warcraft\_classic_era_\Interface\AddOns\QuestieDB` to the intended `Questie-clones/QuestieDB` checkout. Shared Era/Classic provider links were not changed. Questie's existing shared source links remain as they were, so those clients will see consumer edits on their next load; they were not launched or reloaded for this validation.
