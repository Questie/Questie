...
map = {}
for line in requests.get('https://wow.tools/dbc/api/export/?name=areatable&build=2.5.1.38118&locale=enUS').text.split('\n')[1:-1]:
    data = line.split(',')
    if data[3] not in map:
        map[ data[3] ] = []
        map[ data[3] ] += ['[%s]="%s",' % (data[0], data[2])]

for continent in map.keys():
    print('    [%s]={'%continent)
    for string in map[continent]:
        print('        '+string)
    print('    },')
]]
