...
**Removed on 2026-09-02.** `Localization/EntityLocale.lua`, its test, the TOC lines, and the `ApplyExternalLocaleCorrections` call in `Modules/QuestieInit.lua` are gone; `l10n.lua` documents the narrowed `QUESTIE_LOCALES_OVERRIDE` contract (`locale`, `localeName`, `translations`). **Follow-up the same day:** the entity lookup fields are read again, but by `l10n.lua` itself. `InitializeUILocale` forwards `itemLookup`, `questLookup`, `npcNameLookup`, and `objectLookup` to `LibQuestieDB.Corrections.Set` under the owner `QuestieLocalesOverride`, only when the override locale is the effective one, and only for IDs the composed database has. Empty strings and `{}` are skipped because the provider treats them as a blank name and a cleared field. A three-element quest row reads objectives from index 3, so the Ukrainian addon's current data is correct here. The point is that one addon build serves upstream Questie and this branch; an addon may still bypass this and use its own registrar. The rest of this document is the record of what the module did and why it was not moved into the provider.
