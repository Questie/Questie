...
describe("GroupEventHandler", function()
    ...
    describe("isolated comm lifecycle", function()
        ...
        it("uses GROUP_ROSTER_UPDATE to resync when group size changes", function()
            local network = AceCommTestHarness.NewIsolatedNetwork()
            local alice = network:CreateClient({playerName = "Alice", realmName = "TestRealm"})
            local bob = network:CreateClient({playerName = "Bob", realmName = "TestRealm"})
            network:SetParty({alice, bob})
            ...
            assert.are_equal(1, countIsolatedSentAddonMessages(alice, "QuestieH1", "PARTY"))
        ...
        it("resyncs GROUP_ROSTER_UPDATE on online-status changes but not no-op updates", function()
            local network = AceCommTestHarness.NewIsolatedNetwork()
            local alice = network:CreateClient({playerName = "Alice", realmName = "TestRealm"})
            local bob = network:CreateClient({playerName = "Bob", realmName = "TestRealm"})
            network:SetParty({alice, bob})
            ...
            assert.are_equal(2, countIsolatedSentAddonMessages(alice, "QuestieH1", "PARTY"))
