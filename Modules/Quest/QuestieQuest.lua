...
function QuestieQuest:GetRawLeaderBoardDetails(QuestLogIndex)
    ...
    --
    local quest = {}
    local title, level, _, isHeader, _, _, _, questId, _, displayQuestId, _, _, _, _, _, _, _ = GetQuestLogTitle(QuestLogIndex)
    quest.title = title;
    quest.level = level;
    quest.Id = questId;
...
function QuestieQuest:PopulateObjective(Quest, ObjectiveIndex, Objective, BlockItemTooltips) -- must be pcalled
    ...
    if Objective.spawnList then
        ...
        for questId, objectiveTable in pairs(questData) do
            ...
            Questie:Print("Adding objective table to quest", quest.Id)
            quest.objectiveIcons = objectiveTable
            --table.insert(dumpvarr, questData[questId])
        end
        --TODO: Make this better
        --We need to do this because we need to dirty every quest that are in the zone that get modified
        for questId, questData in pairs(QuestiePlayer.currentQuestlog) do
            ...
            for UIMapId, _ in pairs(dirtyMapIds) do
                QuestieFrameNew.utils:RecursiveDirty(quest.objectiveDirty, UIMapId);
            end
        end
...
function QuestieQuest:RefreshAllAvailableQuests()
