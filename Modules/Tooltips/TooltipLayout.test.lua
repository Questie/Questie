...
describe("TooltipLayout", function()
    ...
    it("should preserve AddLine and AddDoubleLine order and packed args", function()
        ...
        assert.are_same("line", tooltip.calls[1].kind)
        assert.are_same("first", tooltip.calls[1].text)
        assert.are_same(3, tooltip.calls[1].n)
        assert.are_same(1, tooltip.calls[1].args[1])
        assert.is_nil(tooltip.calls[1].args[2])
        assert.are_same(3, tooltip.calls[1].args[3])
        assert.are_same("doubleLine", tooltip.calls[2].kind)
        assert.are_same("left", tooltip.calls[2].leftText)
        assert.are_same("right", tooltip.calls[2].rightText)
        assert.are_same(3, tooltip.calls[2].n)
        assert.are_same("line", tooltip.calls[3].kind)
        assert.are_same("last", tooltip.calls[3].text)
        assert.is_nil(capturedTextWrap)
        assert.are_same(0, createFontStringCallCount)
        assert.are_same(0, createFrameCallCount)
    end)
    ...
    it("should wrap descriptions from non-description width minus prefix width", function()
        ...
        assert.are_same(498, capturedTextWrap.desiredWidth)
        assert.are_same(string.rep("B", 900), capturedTextWrap.text)
        assert.are_same("  ", capturedTextWrap.prefix)
        assert.is_false(capturedTextWrap.combineTrailing)
        assert.are_same("  wrapped", tooltip.calls[2].text)
        assert.are_same(3, tooltip.calls[2].n)
    end)
    ...
    it("should render multiple wrapped description lines with the original args", function()
        ...
        assert.are_same("header", tooltip.calls[1].text)
        assert.are_same("  first", tooltip.calls[2].text)
        assert.are_same(3, tooltip.calls[2].n)
        assert.are_same(1, tooltip.calls[2].args[1])
        assert.is_nil(tooltip.calls[2].args[2])
        assert.are_same(3, tooltip.calls[2].args[3])
        assert.are_same("  second", tooltip.calls[3].text)
        assert.are_same(3, tooltip.calls[3].n)
        assert.are_same(1, tooltip.calls[3].args[1])
        assert.is_nil(tooltip.calls[3].args[2])
        assert.are_same(3, tooltip.calls[3].args[3])
    end)
    ...
    it("should use the minimum tooltip width for description-only tooltips", function()
        ...
        assert.are_same(373, capturedTextWrap.desiredWidth)
        assert.is_false(capturedTextWrap.combineTrailing)
    end)
    ...
    it("should include measured double-line gap when deriving description width", function()
        ...
        assert.are_same(449, capturedTextWrap.desiredWidth)
        assert.is_false(capturedTextWrap.combineTrailing)
    end)
    ...
    it("should create raw UI indent textures", function()
        ...
        assert.are_same("|TInterface\\Minimap\\UI-bonusobjectiveblob-inside.blp:1:7.5|t", prefix)
        assert.are_same(7.5, width)
    end)
end)
