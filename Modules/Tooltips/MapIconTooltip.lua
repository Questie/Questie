                ...
                if questData.subData and shift then
                    ...
                    if dataType == "table" then
                        for _, rawLine in pairs(questData.subData) do
                            local lines = QuestieLib:TextWrap(rawLine, "  ", false, true, math.max(375, Tooltip:GetWidth()), questData.questId) --275 is the default questlog width
                            for _, line in pairs(lines) do
                                self:AddLine(line, 0.86, 0.86, 0.86);
                            end
                        end
                    elseif dataType == "string" then
                        local lines = QuestieLib:TextWrap(questData.subData, "  ", false, true, math.max(375, Tooltip:GetWidth())) --275 is the default questlog width
                        for _, line in pairs(lines) do
                            self:AddLine(line, 0.86, 0.86, 0.86);
                        end
                    end
