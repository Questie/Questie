...
`QuestieProfilerEnabled` is set, and a player who never enables it pays one boolean test per loader call and
nothing else.
...
| `Modules/Libs/QuestieLoader.lua` | Not in this folder, but part of the system: it stamps per-file load intervals, which is the only way file load cost is observable at all. |
...
- **`RefreshHooks` excludes `Profiler`, `ProfilerUI` and `ProfilerReport` by module name.** Without the third
  the profiler wraps the report builder and measures itself building the report you are reading.
...
## Reaching code: the three bindings

There are three ways a function can be bound, and each needs a different technique. What decides the technique
...
| A file-local | Nothing short of editing the vendored file | HBD's `UpdateMinimapPins`, `drawMinimapPin` |
...
The third case has no technique and does not need one: the frame-script row gives the total, and reading the
