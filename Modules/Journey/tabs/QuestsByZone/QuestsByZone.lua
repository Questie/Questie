...
function _QuestieJourney.questsByZone:CollectZoneQuests(zoneId)
    ...
    for _, levelAndQuest in pairs(sortedQuestByLevel) do
        ...
        if hiddenQuests and (((not hiddenQuests[questId]) or hiddenQuests[questId] == HIDE_ON_MAP) or QuestieEvent:IsEventQuest(questId)) and QuestieDB.LibQuestieDB.Quest.GetAllIds(true)[questId] then --TODO: Fix this, do not return the entire list every time for the Ids
            temp.value = questId
