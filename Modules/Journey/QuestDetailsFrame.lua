...
function _QuestieJourney:DrawQuestDetailsFrame(container, quest)
    ...
    local questInfoHeader = _QuestieJourney:CreateHeading(i10n('Quest Information'), true)
    container:AddChild(questInfoHeader)

    -- Generic Quest Information

    local levelLabel = _QuestieJourney:CreateLabel(Questie:Colorize(i10n('Recommended Quest Level: '), 'yellow') .. quest.level, true)
    container:AddChild(levelLabel)

    local minLevelLabel = _QuestieJourney:CreateLabel(Questie:Colorize(i10n('Minimum Required Level for Quest: '), 'yellow') .. quest.requiredLevel, true)
    ...
    local questIdLabel = _QuestieJourney:CreateLabel(Questie:Colorize(i10n('Quest ID: '), 'yellow') .. quest.Id, true)
    ...
    if quest.Starts and quest.Starts.NPC then
        ...
        startNPCGroup:SetTitle(i10n('Quest Start NPC Information'))
        ...
        if startNpc.questStarts then
            ...
            alsoStartsLabel:SetText(i10n('This NPC Also Starts the following quests:'))
            ...
            if #startQuests == 0 then
                ...
                noQuestLabel:SetText(i10n('No Quests to List'))
    ...
    if quest.Starts and quest.Starts.GameObject then
        ...
        startObjectGroup:SetTitle(i10n('Quest Start Object Information'))
        ...
        for _, oid in pairs(quest.Starts.GameObject) do
            ...
            if startObj.questStarts then
                ...
                local alsoStartsLabel = AceGUI:Create("Label")
                alsoStartsLabel:SetText(i10n('This Object Also Starts the following quests:'))
                alsoStartsLabel:SetColor(255, 165, 0)
                alsoStartsLabel:SetFontObject(GameFontHighlight)
                alsoStartsLabel:SetFullWidth(true)
    ...
    if quest.Finisher and quest.Finisher.Name and quest.Finisher.Type == "monster" then
        ...
        endNPCGroup:SetTitle(i10n('Quest Turn-in NPC Information'))
        ...
        if endNPC.endQuests then
            ...
            alsoEndsLabel:SetText(i10n('This NPC Also Completes the following quests:'))
...
function _QuestieJourney:GetDifficultyString(questLevel, questMinLevel)
    ...
    return Questie:Colorize(i10n('Difficulty Range: %s', diffStr), 'yellow')
end
...
function _QuestieJourney:CreatePreQuestGroup(quest)
    ...
    preQuestInlineGroup:SetTitle(i10n('Pre Quests'))
