---@type l10n
local l10n = QuestieLoader:ImportModule("l10n")

local tooltipOptionsLocales = {
    ["Tooltips"] = {
        ["enUS"] = true,
        ["deDE"] = "Tooltip",
        ["esES"] = "Tooltips",
        ["esMX"] = "Tooltips",
        ["frFR"] = "Infobulle",
        ["itIT"] = "Tooltip",
        ["koKR"] = "툴팁",
        ["ptBR"] = "Dicas de ferramentas",
        ["ruRU"] = "Подсказки",
        ["zhCN"] = "工具提示框",
        ["zhTW"] = "浮動提示資訊",
    },
    ["Tooltip Options"] = {
        ["enUS"] = true,
        ["deDE"] = "Tooltip-Optionen",
        ["esES"] = "Opciones de tooltip",
        ["esMX"] = "Opciones de tooltip",
        ["frFR"] = "Options des infobulles",
        ["itIT"] = "Opzioni dei tooltip",
        ["koKR"] = "툴팁 옵션",
        ["ptBR"] = "Opções de dica de ferramenta",
        ["ruRU"] = "Настройки всплывающих подсказок",
        ["zhCN"] = "工具提示框选项",
        ["zhTW"] = "浮動提示資訊選項",
    },
    ["Only show party members"] = {
        ["enUS"] = true,
        ["deDE"] = "Party nur geteilt",
        ["esES"] = "Mostrar solo miembros del grupo",
        ["esMX"] = "Mostrar solo miembros del grupo",
        ["frFR"] = "Partage avec le groupe uniquement",
        ["itIT"] = "Mostrare solo i membri del gruppo",
        ["koKR"] = "파티만 공유",
        ["ptBR"] = "Apenas mostrar membros do grupo",
        ["ruRU"] = "Только от игроков группы",
        ["zhCN"] = "仅限队友",
        ["zhTW"] = "只顯示隊友的任務",
    },
    ["If checked, shared quest info will only show players in your party."] = {
        ["enUS"] = true,
        ["deDE"] = "Wenn dies aktiviert ist, werden freigegebene Questinformationen nur angezeigt, wenn sie von Gruppenmitgliedern stammen.",
        ["esES"] = "Si está seleccionado, la información de misiones compartidas solo se mostrará si son de miembros del grupo.",
        ["esMX"] = "Si está seleccionado, la información de misiones compartidas solo se mostrará si son de miembros del grupo.",
        ["frFR"] = "Si cette option est cochée, les informations des quêtes partagées ne seront affichées qu’aux joueurs de votre groupe.",
        ["itIT"] = "Se selezionato, le informazioni condivise sulle missioni verranno mostrate solo per i giocatori del tuo gruppo.",
        ["koKR"] = "이 옵션을 선택하면 공유 퀘스트 정보가 파티원의 정보인 경우에만 표시됩니다.",
        ["ptBR"] = "Se marcado, as informações compartilhadas das missões serão mostradas apenas se forem de membros do grupo.",
        ["ruRU"] = "Когда включено, информация о таком же, как у вас, задании будет отображаться только от членов группы",
        ["zhCN"] = "启用时，仅显示您队伍中的玩家的共享任务信息",
        ["zhTW"] = "啟用時，只會分享隊伍中玩家的任務資訊",
    },
    ["Show quests in NPC tooltips"] = {
        ["enUS"] = true,
        ["deDE"] = "Quests in NPC Tooltips anzeigen",
        ["esES"] = "Mostrar misiones en el tooltip",
        ["esMX"] = "Mostrar misiones en el tooltip",
        ["frFR"] = "Afficher les quêtes dans les infobulles",
        ["itIT"] = "Mostrare le missioni nei tooltip degli NPC",
        ["koKR"] = "NPC 툴팁에 퀘스트 표시",
        ["ptBR"] = "Mostrar missões nas dicas",
        ["ruRU"] = "Задания в подсказках NPC",
        ["zhCN"] = "显示NPC提示框中的任务",
        ["zhTW"] = "顯示 NPC 相關任務提示",
    },
    ["Show quests (available/complete) in the NPC tooltips."] = {
        ["enUS"] = true,
        ["deDE"] = "Zeigt Quests (verfügbar/abgeschlossen) in den NPC Tooltips.",
        ["esES"] = "Muestra misiones (disponibles/completadas) en el tooltip del PNJ.",
        ["esMX"] = "Muestra misiones (disponibles/completadas) en el tooltip del PNJ.",
        ["frFR"] = "Affiche les quêtes (disponibles / terminées) dans les infobulles des PNJ.",
        ["itIT"] = "Mostra le missioni (disponibili/completate) nei tooltip degli NPC.",
        ["koKR"] = "NPC 툴팁에 (가능한/완료한) 퀘스트표시",
        ["ptBR"] = "Mostra missões (disponíveis/concluídas) nas dicas de ferramentas do NPC.",
        ["ruRU"] = "Показать/скрыть задания (доступные/выполненные) во всплывающих подсказках NPC",
        ["zhCN"] = "在NPC提示框中显示任务（可接/完成）",
        ["zhTW"] = "在 NPC 的浮動提示資訊中顯示任務 (可接/完成)",
    },
    ["Show quest XP at max level"] = {
        ["enUS"] = true,
        ["deDE"] = "Zeige XP auf Max-Level",
        ["esES"] = "Mostrar EXP al nivel máximo",
        ["esMX"] = "Mostrar EXP al nivel máximo",
        ["frFR"] = "Afficher le PX au niveau maximum",
        ["itIT"] = "Mostrare l'XP delle missioni al livello massimo",
        ["koKR"] = "최대 레벨에서 퀘스트 경험치 표시",
        ["ptBR"] = "Mostrar EXP no nível máximo",
        ["ruRU"] = "Опыт на макс. уровне",
        ["zhCN"] = "满级时显示任务经验值",
        ["zhTW"] = "滿等時顯示任務經驗值",
    },
    ["Shows the quest XP values on quests even at max level."] = {
        ["enUS"] = true,
        ["deDE"] = "Zeigt die Quest XP auch auf Max-Level an.",
        ["esES"] = "Muestra los valores de experiencia de misiones incluso al nivel máximo.",
        ["esMX"] = "Muestra los valores de experiencia de misiones incluso al nivel máximo.",
        ["frFR"] = "Affiche le montant d'expérience des quêtes, même au niveau maximum.",
        ["itIT"] = "Mostra i valori di XP delle missioni anche al livello massimo.",
        ["koKR"] = "최대 레벨에서도 퀘스트 경험치 값을 표시합니다.",
        ["ptBR"] = "Mostra os valores de experiência da missão mesmo no nível máximo.",
        ["ruRU"] = "Показывать опыт за задания даже на максимальном уровне персонажа",
        ["zhCN"] = "即使满级时也显示任务经验值",
        ["zhTW"] = "滿等後也要顯示任務的經驗值",
    },
    ["Next in chain"] = {
        ["enUS"] = true,
        ["deDE"] = "Folge-Quests in der Reihe",
        ["esES"] = "Siguiente misión",
        ["esMX"] = "Siguiente misión",
        ["frFR"] = "Prochaine quête",
        ["itIT"] = "Prossima missione",
        ["koKR"] = "다음 연계 퀘스트",
        ["ptBR"] = "Próxima missão",
        ["ruRU"] = "Следующее в цепочке",
        ["zhCN"] = "后续任务",
        ["zhTW"] = "後續任務",
    },
    ["Show next quests in chain"] = {
        ["enUS"] = true,
        ["deDE"] = "Zeige nächste Quests der Reihe",
        ["esES"] = "Mostrar siguientes misiones",
        ["esMX"] = "Mostrar siguientes misiones",
        ["frFR"] = "Afficher les prochaines quêtes",
        ["itIT"] = "Mostrare le prossime missioni della catena",
        ["koKR"] = "연계 퀘스트 표시",
        ["ptBR"] = "Mostrar próximas missões",
        ["ruRU"] = "Следующие задания в цепочке",
        ["zhCN"] = "显示后续任务",
        ["zhTW"] = "顯示後續任務",
    },
    ["Show breadcrumb quests"] = {
        ["enUS"] = true,
        ["deDE"] = "Zeige Brotkrumen-Quests",
        ["esES"] = "Mostrar misiones de rastro",
        ["esMX"] = "Mostrar misiones de migajas",
        ["frFR"] = "Afficher les quêtes d'orientation",
        ["itIT"] = "Mostrare le missioni guida",
        ["koKR"] = "길잡이 퀘스트 표시",
        ["ptBR"] = "Mostrar missões de encaminhamento",
        ["ruRU"] = "Показать предзадания",
        ["zhCN"] = "显示线索任务",
        ["zhTW"] = "顯示任務線索",
    },
    ["If checked, breadcrumb quests will show in the expanded map tooltips."] = {
        ["enUS"] = true,
        ["deDE"] = "Wenn ausgewählt, werden Brotkrumen-Quests in den erweiterten Map-Tooltips angezeigt.",
        ["esES"] = "Si está seleccionado, las misiones de rastro se mostrarán en los tooltips de mapa expandidos.",
        ["esMX"] = "Si está seleccionado, las misiones de migajas se mostrarán en los tooltips de mapa expandidos.",
        ["frFR"] = "Si cette option est cochée, les quêtes d'orientation seront affichées dans les infobulles étendues de la carte.",
        ["itIT"] = "Se selezionato, le missioni guida verranno mostrate nei tooltip espansi della mappa.",
        ["koKR"] = "이 옵션을 선택하면 길잡이 퀘스트가 확장된 지도 툴팁에 표시됩니다.",
        ["ptBR"] = "Se marcado, as missões de encaminhamento serão mostradas nas dicas de mapa expandidas.",
        ["ruRU"] = "Когда включено, предзадания будут отображаться в расширенной подсказке на карте",
        ["zhCN"] = "启用时，线索任务会在展开的地图提示框中显示",
        ["zhTW"] = "啟用時，會在展開的地圖浮動提示資訊中顯示任務線索",
    },
    ["If checked, the next quests in the chain will show in the expanded map tooltips."] = {
        ["enUS"] = true,
        ["deDE"] = "Wenn ausgewählt, werden die nächsten Quests der Reihe in den erweiterten Map-Tooltips angezeigt.",
        ["esES"] = "Si está seleccionado, las siguientes misiones en la serie se mostrarán en los tooltips de mapa expandidos.",
        ["esMX"] = "Si está seleccionado, las siguientes misiones en la serie se mostrarán en los tooltips de mapa expandidos.",
        ["frFR"] = "Si cette option est cochée, les prochaines quêtes de la suite seront affichées dans les infobulles étendues de la carte.",
        ["itIT"] = "Se selezionato, le prossime missioni della catena verranno mostrate nei tooltip espansi della mappa.",
        ["koKR"] = "이 옵션을 선택하면 연계 퀘스트가 확장된 지도 툴팁에 표시됩니다.",
        ["ptBR"] = "Se marcado, as próximas missões na série serão mostradas nas dicas de mapa expandidas.",
        ["ruRU"] = "Когда включено, следующие задания в цепочке будут отображаться в расширенной подсказке на карте",
        ["zhCN"] = "启用时，任务链中的后续任务会在展开的地图提示框中显示",
        ["zhTW"] = "啟用時，會在展開的地圖浮動提示資訊中顯示任務串中的下一個任務",
    },
    ["Show item drop rates"] = {
        ["enUS"] = true,
        ["deDE"] = "Zeige Gegenstand-Drop-Rate",
        ["esES"] = "Mostrar probabilidad de obtención",
        ["esMX"] = "Mostrar probabilidad de obtención",
        ["frFR"] = "Afficher les taux d'obtention",
        ["itIT"] = "Mostrare i tassi di drop degli oggetti",
        ["koKR"] = "아이템 드랍율 표시",
        ["ptBR"] = "Mostrar chance de saque",
        ["ruRU"] = "Шанс выпадения предмета",
        ["zhCN"] = "显示物品掉率",
        ["zhTW"] = "顯示物品掉落率",
    },
    ["If checked, drop rates for quest objectives will show on mob tooltips."] = {
        ["enUS"] = true,
        ["deDE"] = "Aktiviert/Deaktiviert die Drop-Rate für Quest-Gegenstände in den Tooltips von Gegnern.",
        ["esES"] = "Si está seleccionado, la probabilidad de obtención de los objetivos de misión se mostrará en los tooltips de los enemigos.",
        ["esMX"] = "Si está seleccionado, la probabilidad de obtención de los objetivos de misión se mostrará en los tooltips de los enemigos.",
        ["frFR"] = "Si cette option est cochée, les taux d'obtention des objectifs de quête s'affichent dans les infobulles des ennemis.",
        ["itIT"] = "Se selezionato, le probabilità di drop degli obiettivi di missione verranno mostrate nei tooltip dei mob.",
        ["koKR"] = "이 옵션을 선택하면 퀘스트 목표 아이템의 드롭률이 몬스터 툴팁 정보에 표시됩니다.",
        ["ptBR"] = "Se marcado, a chance de saque dos objetivos de missão será mostrada nas dicas dos inimigos.",
        ["ruRU"] = "Когда включено, шанс выпадения предметов для выполнения задания будет отображаться во всплывающей подсказке",
        ["zhCN"] = "启用时，任务目标的掉率将显示在怪物信息提示框中",
        ["zhTW"] = "啟用時，任務目標的掉落率將顯示在怪物的提示資訊中",
    },
}

for k, v in pairs(tooltipOptionsLocales) do
    l10n.translations[k] = v
end
