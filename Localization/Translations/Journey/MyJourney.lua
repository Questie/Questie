---@type l10n
local l10n = QuestieLoader:ImportModule("l10n")

local myJourneyLocales = {
    ["Questie Options"] = {
        ["enUS"] = true,
        ["deDE"] = "Questie-Einstellungen",
        ["esES"] = "Opciones",
        ["esMX"] = "Opciones",
        ["frFR"] = "Options",
        ["itIT"] = "Opzioni",
        ["koKR"] = "Questie 설정",
        ["ptBR"] = "Opções",
        ["ruRU"] = "Настройки Questie",
        ["zhCN"] = "Questie选项",
        ["zhTW"] = "設定選項",
    },
    ["My Journey"] = {
        ["enUS"] = true,
        ["deDE"] = "Meine Reise",
        ["esES"] = "Mi viaje",
        ["esMX"] = "Mi viaje",
        ["frFR"] = "Mon voyage",
        ["itIT"] = "Il mio viaggio",
        ["koKR"] = "나의 여정",
        ["ptBR"] = "Minha viagem",
        ["ruRU"] = "Путешествие",
        ["zhCN"] = "我的旅程",
        ["zhTW"] = "我的冒險日記",
    },
    ["%s's Journey"] = {
        ["enUS"] = true,
        ["deDE"] = "%s's Reise",
        ["esES"] = "Viaje de %s",
        ["esMX"] = "Viaje de %s",
        ["frFR"] = "Le voyage de %s",
        ["itIT"] = "Il viaggio di %s",
        ["koKR"] = "%s의 여정",
        ["ptBR"] = "Viagem de %s",
        ["ruRU"] = "Путешествие персонажа %s",
        ["zhCN"] = "%s的旅程",
        ["zhTW"] = "%s的冒險日記",
    },
    ["Your Recent History"] = {
        ["enUS"] = true,
        ["deDE"] = "Deine aktuelle Historie",
        ["esES"] = "Tu historial reciente",
        ["esMX"] = "Tu historial reciente",
        ["frFR"] = "Votre historique récent",
        ["itIT"] = "La tua cronologia recente",
        ["koKR"] = "최근 기록",
        ["ptBR"] = "Seu histórico recente",
        ["ruRU"] = "Недавняя история",
        ["zhCN"] = "你的近期纪录",
        ["zhTW"] = "最近的歷史記錄",
    },
    ["It's about time you embark on your first Journey!"] = {
        ["enUS"] = true,
        ["deDE"] = "Es ist an der Zeit, dass du dich auf deine erste Reise begibst!",
        ["esES"] = "¡Es hora de que te embarques en tu primer viaje!",
        ["esMX"] = "¡Es hora de que te embarques en tu primer viaje!",
        ["frFR"] = "Il est temps d'embarquer pour votre premier voyage !",
        ["itIT"] = "È ora di intraprendere il tuo primo viaggio!",
        ["koKR"] = "당신은 이제 막 첫번째 여정을 시작했습니다!",
        ["ptBR"] = "É hora de embarcar em sua primeira viagem!",
        ["ruRU"] = "Пора начинать свое первое путешествие!",
        ["zhCN"] = "是时候开启你的首次旅程了！",
        ["zhTW"] = "該是你踏上第一次旅程的時候了！",
    },
    ["Congratulations! You reached %s !"] = {
        ["enUS"] = true,
        ["deDE"] = "Glückwunsch! Ihr habt %s erreicht!",
        ["esES"] = "¡Enhorabuena! ¡Has alcanzado el %s!",
        ["esMX"] = "¡Felicitaciones, has alcanzado el %s!",
        ["frFR"] = "Félicitations, vous avez atteint le %s !",
        ["itIT"] = "Congratulazioni! Hai raggiunto il %s !",
        ["koKR"] = "축하합니다. %s 되었습니다!",
        ["ptBR"] = "Parabéns, você atingiu o %s!",
        ["ruRU"] = "Поздравляем, вы достигли %s!",
        ["zhCN"] = "祝贺你，你升到了%s！",
        ["zhTW"] = "恭喜你，你升到了%s！",
    },
    ["You Accepted the quest %s"] = {
        ["enUS"] = true,
        ["deDE"] = "Du hast die Quest '%s' angenommen",
        ["esES"] = "Aceptaste la misión %s",
        ["esMX"] = "Aceptaste la misión %s",
        ["frFR"] = "Vous avez débuté la quête %s",
        ["itIT"] = "Hai accettato la missione %s",
        ["koKR"] = "%s 퀘스트를 수락했습니다",
        ["ptBR"] = "Você aceitou a missão %s",
        ["ruRU"] = "Задание принято: %s",
        ["zhCN"] = "你接受了任务：%s",
        ["zhTW"] = "已接受任務 %s",
    },
    ["Quest %s: %s"] = {
        ["enUS"] = true,
        ["deDE"] = "Quest %s: %s",
        ["esES"] = "Misión %s: %s",
        ["esMX"] = "Misión %s: %s",
        ["frFR"] = "Quête %s : %s",
        ["itIT"] = "Missione %s: %s",
        ["koKR"] = "퀘스트 %s: %s",
        ["ptBR"] = "Missão %s: %s",
        ["ruRU"] = "Задание %s: %s",
        ["zhCN"] = "任务%s：%s",
        ["zhTW"] = "任務 %s：%s",
    },
    ["Level %s"] = {
        ["enUS"] = true,
        ["deDE"] = "Stufe %s",
        ["esES"] = "nivel %s",
        ["esMX"] = "nivel %s",
        ["frFR"] = "niveau %s",
        ["itIT"] = "livello %s",
        ["koKR"] = "%s 레벨이",
        ["ptBR"] = "nível %s",
        ["ruRU"] = "%s-го уровня",
        ["zhCN"] = "%s级",
        ["zhTW"] = "%s級",
    },
    ["You Reached Level %s"] = {
        ["enUS"] = true,
        ["deDE"] = "Ihr habt Stufe %s erreicht",
        ["esES"] = "Has alcanzado el nivel %s",
        ["esMX"] = "Has alcanzado el nivel %s",
        ["frFR"] = "Vous avez atteint le niveau %s",
        ["itIT"] = "Hai raggiunto il livello %s",
        ["koKR"] = "%s 레벨이 되었습니다",
        ["ptBR"] = "Você atingiu o nível %s",
        ["ruRU"] = "вы достигли %s-го уровня",
        ["zhCN"] = "你升到了%s级",
        ["zhTW"] = "你升到了%s級",
    },
    ["Accepted"] = {
        ["enUS"] = true,
        ["deDE"] = "Angenommen",
        ["esES"] = "Aceptada",
        ["esMX"] = "Aceptada",
        ["frFR"] = "Acceptée",
        ["itIT"] = "Accettata",
        ["koKR"] = "수락",
        ["ptBR"] = "Aceita",
        ["ruRU"] = "принято",
        ["zhCN"] = "接受",
        ["zhTW"] = "接受",
    },
    ["You Abandoned the quest %s"] = {
        ["enUS"] = true,
        ["deDE"] = "Du hast die Quest '%s' abgebrochen",
        ["esES"] = "Abandonaste la misión %s",
        ["esMX"] = "Abandonaste la misión %s",
        ["frFR"] = "Vous avez abandonné la quête %s",
        ["itIT"] = "Hai abbandonato la missione %s",
        ["koKR"] = "%s 퀘스트를 포기했습니다",
        ["ptBR"] = "Você abandonou a missão %s",
        ["ruRU"] = "Задание отменено: %s",
        ["zhCN"] = "你放弃了任务：%s",
        ["zhTW"] = "已放棄任務 %s",
    },
    ["Abandoned"] = {
        ["enUS"] = true,
        ["deDE"] = "Abgebrochen",
        ["esES"] = "Abandonada",
        ["esMX"] = "Abandonada",
        ["frFR"] = "Abandonnée",
        ["itIT"] = "Abbandonata",
        ["koKR"] = "포기",
        ["ptBR"] = "Abandonada",
        ["ruRU"] = "отменено",
        ["zhCN"] = "放弃",
        ["zhTW"] = "放棄",
    },
    ["Entry Title"] = {
        ["enUS"] = true,
        ["deDE"] = "Titel des Eintrags",
        ["esES"] = "Título",
        ["esMX"] = "Título",
        ["frFR"] = "Titre :",
        ["itIT"] = "Titolo",
        ["koKR"] = "제목",
        ["ptBR"] = "Título",
        ["ruRU"] = "Название",
        ["zhCN"] = "输入标题",
        ["zhTW"] = "輸入標題",
    },
    ["Journal Entry"] = {
        ["enUS"] = true,
        ["deDE"] = "Tagebucheintrag",
        ["esES"] = "Entrada de viaje",
        ["esMX"] = "Entrada de viaje",
        ["frFR"] = "Message :",
        ["itIT"] = "Voce di viaggio",
        ["koKR"] = "내용",
        ["ptBR"] = "Entrada da viagem",
        ["ruRU"] = "Описание",
        ["zhCN"] = "输入游记",
        ["zhTW"] = "冒險筆記內容",
    },
    ["You Completed the quest %s"] = {
        ["enUS"] = true,
        ["deDE"] = "Du hast die Quest '%s' abgeschlossen",
        ["esES"] = "Completaste la misión %s",
        ["esMX"] = "Completaste la misión %s",
        ["frFR"] = "Vous avez terminé la quête %s",
        ["itIT"] = "Hai completato la missione %s",
        ["koKR"] = "%s 퀘스트를 완료했습니다",
        ["ptBR"] = "Você concluiu a missão %s",
        ["ruRU"] = "Задание выполнено: %s",
        ["zhCN"] = "你完成了任务：%s",
        ["zhTW"] = "已完成任務 %s",
    },
    ["Add Entry"] = {
        ["enUS"] = true,
        ["deDE"] = "Eintrag hinzufügen",
        ["esES"] = "Añadir entrada",
        ["esMX"] = "Añadir entrada",
        ["frFR"] = "Enregistrer la note",
        ["itIT"] = "Aggiungere voce",
        ["koKR"] = "메모 추가",
        ["ptBR"] = "Adicionar entrada",
        ["ruRU"] = "Добавить заметку",
        ["zhCN"] = "添加条目",
        ["zhTW"] = "新增內容",
    },
    ["Note Created: %s"] = {
        ["enUS"] = true,
        ["deDE"] = "Notiz erstellt: %s",
        ["esES"] = "Nota creada: %s",
        ["esMX"] = "Nota creada: %s",
        ["frFR"] = "Note créée : %s",
        ["itIT"] = "Nota creata: %s",
        ["koKR"] = "메모 추가: %s",
        ["ptBR"] = "Anotação criada: %s",
        ["ruRU"] = "Заметка добавлена: %s",
        ["zhCN"] = "笔记创建于：%s",
        ["zhTW"] = "筆記建立於：%s",
    },
    ["No Note was entered. You must enter a note before submitting."] = {
        ["enUS"] = true,
        ["deDE"] = "Keine Notiz angegeben. Du musst eine Notiz angeben, bevor ein Eintrag angelegt werden kann.",
        ["esES"] = "No has introducido una nota. Tienes que introducir uno antes de crear tu nota.",
        ["esMX"] = "No has introducido una nota. Tienes que introducir una antes de crear tu nota.",
        ["frFR"] = "Aucun message n'a été trouvé. Vous devez entrer un message avant d'enregistrer.",
        ["itIT"] = "Non è stata inserita alcuna nota. Devi inserire una nota prima di inviare.",
        ["koKR"] = "내용이 입력되지 않았습니다. 메모를 추가하려면 내용을 입력해주세요.",
        ["ptBR"] = "Nenhuma anotação foi inserida. Você deve inserir uma anotação antes de submeter.",
        ["ruRU"] = "Вы не ввели описание заметки. Введите описание для добавления заметки.",
        ["zhCN"] = "未输入备注，在提交前你必须输入一个备注",
        ["zhTW"] = "沒有輸入筆記，送出前必須先輸入筆記內容",
    },
    ["No Title was entered. You must enter a title before submitting your note."] = {
        ["enUS"] = true,
        ["deDE"] = "Kein Titel angegeben. Du musst einen Titel angeben, bevor ein Eintrag angelegt werden kann.",
        ["esES"] = "No has introducido un título. Tienes que introducir uno antes de crear tu nota.",
        ["esMX"] = "No has introducido un título. Tienes que introducir uno antes de crear tu nota.",
        ["frFR"] = "Aucun titre n'a été trouvé. Vous devez entrer un titre avant d'enregistrer.",
        ["itIT"] = "Non è stato inserito alcun titolo. Devi inserire un titolo prima di inviare la nota.",
        ["koKR"] = "제목이 입력되지 않았습니다. 메모를 추가하려면 제목을 입력해주세요.",
        ["ptBR"] = "Nenhum título foi inserido. Você deve inserir um título antes de submeter sua anotação.",
        ["ruRU"] = "Вы не ввели название заметки. Введите название для добавления заметки.",
        ["zhCN"] = "未输入标题，在提交前你必须输入一个标题",
        ["zhTW"] = "沒有輸入標題，送出筆記前必須先輸入標題",
    },
    ["New Note"] = {
        ["enUS"] = true,
        ["deDE"] = "Neue Notiz",
        ["esES"] = "Nueva nota",
        ["esMX"] = "Nueva nota",
        ["frFR"] = "Nouvelle note",
        ["itIT"] = "Nuova nota",
        ["koKR"] = "새 메모",
        ["ptBR"] = "Nova anotação",
        ["ruRU"] = "Новая заметка",
        ["zhCN"] = "新笔记",
        ["zhTW"] = "新筆記",
    },
    ["Note: %s"] = {
        ["enUS"] = true,
        ["deDE"] = "Notiz: %s",
        ["esES"] = "Nota: %s",
        ["esMX"] = "Nota: %s",
        ["frFR"] = "Note : %s",
        ["itIT"] = "Nota: %s",
        ["koKR"] = "메모: %s",
        ["ptBR"] = "Anotação: %s",
        ["ruRU"] = "Заметка: %s",
        ["zhCN"] = "笔记：%s",
        ["zhTW"] = "筆記：%s",
    },
    ["Add New Adventure Note"] = {
        ["enUS"] = true,
        ["deDE"] = "Neue Abenteuer-Notiz hinzufügen",
        ["esES"] = "Añadir nota",
        ["esMX"] = "Añadir nota",
        ["frFR"] = "Ajouter une note",
        ["itIT"] = "Aggiungere nota",
        ["koKR"] = "새 여행 메모 추가",
        ["ptBR"] = "Adicionar nota",
        ["ruRU"] = "Новая заметка",
        ["zhCN"] = "添加冒险笔记",
        ["zhTW"] = "新增冒險筆記",
    },
    ["Create an entry in your journal to remember a specific moment. Simply supply a title and description and Questie will remember it for you!"] = {
        ["enUS"] = true,
        ["deDE"] = "Erstelle einen Eintrag in deinem Reisetagebuch, um dich an einen bestimmten Moment zu erinnern. Gebe einfach einen Titel und eine Beschreibung an und Questie wird sich für dich erinnern!",
        ["esES"] = "Crea una nueva entrada en el tu viaje para recordar un momento especifico. Simplemente proporciona un titulo y una descripción y Questie lo recordará por ti",
        ["esMX"] = "Crea una nueva nota en tu viaje para recordar un momento especifico. Simplemente proporciona un titulo y una descripción y Questie lo recordará por ti",
        ["frFR"] = "Créez une note dans votre journal pour vous rappeler un moment précis. Indiquez simplement un titre et une description et Questie s'en souviendra pour vous !",
        ["itIT"] = "Crea una voce nel tuo diario per ricordare un momento specifico. Basta fornire un titolo e una descrizione e Questie lo ricorderà per te!",
        ["koKR"] = "여정 내역에 메모를 남겨 특별한 순간을 기억해보세요. 간단하게 제목과 내용을 입력하면 Questie가 당신을 위해 기억해드립니다!",
        ["ptBR"] = "Cria uma entrada no seu diário para lembrá-lo de um momento específico. Basta fornecer um título e uma descrição e o Questie lembrará disso para você!",
        ["ruRU"] = "Создает заметку в журнале путешествия, чтобы запомнить определенный момент. Просто введите название и описание, и Questie запомнит это для вас!",
        ["zhCN"] = "替你的魔兽升级之旅创建一个条目，纪录特别的时刻。只要输入标题和内容，Questie就会替你保存下来！",
        ["zhTW"] = "在你的冒險日記中建立新內容來記錄特別的時刻，只要簡單的輸入標題和內容描述，任務位置提示插件就會幫你保存起來！",
    },
    ["Delete Note"] = {
        ["enUS"] = true,
        ["deDE"] = "Notiz löschen",
        ["esES"] = "Eliminar nota",
        ["esMX"] = "Eliminar nota",
        ["frFR"] = "Supprimer la note",
        ["itIT"] = "Eliminare nota",
        ["koKR"] = "메모 삭제",
        ["ptBR"] = "Excluir anotação",
        ["ruRU"] = "Удалить заметку",
        ["zhCN"] = "删除笔记",
        ["zhTW"] = "刪除筆記",
    },
    ["Are you sure you want to delete this note?"] = {
        ["enUS"] = true,
        ["deDE"] = "Bist du sicher, dass du diese Notiz löschen möchtest?",
        ["esES"] = "¿Estás seguro de que quieres eliminar esta nota?",
        ["esMX"] = "¿Estás seguro de que quieres eliminar esta nota?",
        ["frFR"] = "Êtes-vous sûr de vouloir supprimer cette note ?",
        ["itIT"] = "Sei sicuro di voler eliminare questa nota?",
        ["koKR"] = "정말로 이 메모를 삭제하시겠습니까?",
        ["ptBR"] = "Tem certeza de que deseja excluir esta anotação?",
        ["ruRU"] = "Вы уверены, что хотите удалить эту заметку?",
        ["zhCN"] = "确定要删除这个笔记吗？",
        ["zhTW"] = "確定要刪除這個筆記嗎？",
    },
    ["Note deleted successfully"] = {
        ["enUS"] = true,
        ["deDE"] = "Notiz erfolgreich gelöscht",
        ["esES"] = "Nota eliminada con éxito",
        ["esMX"] = "Nota eliminada con éxito",
        ["frFR"] = "Note supprimée avec succès",
        ["itIT"] = "Nota eliminata con successo",
        ["koKR"] = "메모가 성공적으로 삭제되었습니다",
        ["ptBR"] = "Anotação excluída com sucesso",
        ["ruRU"] = "Заметка успешно удалена",
        ["zhCN"] = "笔记删除成功",
        ["zhTW"] = "筆記刪除成功",
    },
    ["Character re-creation detected, resetting \"My Journey\" data."] = {
        ["enUS"] = true,
        ["deDE"] = "Charakter-Neuerstellung erkannt, \"Meine Reise\"-Daten werden zurückgesetzt.",
        ["esES"] = "Recreación de personaje detectada, restableciendo los datos de 'Mi viaje'",
        ["esMX"] = "Recreación de personaje detectada, restableciendo los datos de 'Mi viaje'",
        ["frFR"] = "Recréation de personnage détectée, réinitialisation des données de 'Mon voyage'",
        ["itIT"] = "Ricreazione del personaggio rilevata, reimpostazione dei dati di 'Il mio viaggio'.",
        ["koKR"] = "캐릭터 재생성이 감지되어 여정 데이터가 초기화됩니다",
        ["ptBR"] = "Recriação de personagem detectada, redefinindo os dados de 'Minha viagem'",
        ["ruRU"] = "Обнаружено пересоздание персонажа, сброс данных Путешествия.",
        ["zhCN"] = "检测到角色重建，正在重置旅途数据",
        ["zhTW"] = "已偵測到角色重建，旅途資料重置中",
    },
    ["Ineligible"] = {
        ["enUS"] = true,
        ["deDE"] = "Nicht berechtigt",
        ["esES"] = "No elegible",
        ["esMX"] = "No elegible",
        ["frFR"] = "Non éligible",
        ["itIT"] = "Non idoneo",
        ["koKR"] = "자격 없음",
        ["ptBR"] = "Não elegível",
        ["ruRU"] = "Недоступно",
        ["zhCN"] = "不符合条件",
        ["zhTW"] = "不符合條件",
    },
    ["Eligible"] = {
        ["enUS"] = true,
        ["deDE"] = "Berechtigt",
        ["esES"] = true,
        ["esMX"] = true,
        ["frFR"] = "Éligible",
        ["itIT"] = "Idoneo",
        ["koKR"] = "자격 있음",
        ["ptBR"] = "Elegível",
        ["ruRU"] = "Доступно",
        ["zhCN"] = "符合条件",
        ["zhTW"] = "符合條件",
    },
    ["Already complete"] = {
        ["enUS"] = true,
        ["deDE"] = "Bereits abgeschlossen",
        ["esES"] = "Ya completado",
        ["esMX"] = "Ya completado",
        ["frFR"] = "Déjà terminé",
        ["itIT"] = "Già completato",
        ["koKR"] = "이미 완료됨",
        ["ptBR"] = "Já concluído",
        ["ruRU"] = "Уже выполнено",
        ["zhCN"] = "已完成",
        ["zhTW"] = "已完成",
    },
    ["Player is on quest"] = {
        ["enUS"] = true,
        ["deDE"] = "Spieler ist auf der Quest",
        ["esES"] = "El jugador está en la misión",
        ["esMX"] = "El jugador está en la misión",
        ["frFR"] = "Le joueur est en quête",
        ["itIT"] = "Il giocatore è in missione",
        ["koKR"] = "플레이어가 퀘스트 수행 중",
        ["ptBR"] = "Jogador está na missão",
        ["ruRU"] = "Игрок выполняет задание",
        ["zhCN"] = "玩家正在进行任务",
        ["zhTW"] = "玩家正在進行任務",
    },
    ["Unknown"] = {
        ["enUS"] = true,
        ["deDE"] = "Unbekannt",
        ["esES"] = "Desconocido",
        ["esMX"] = "Desconocido",
        ["frFR"] = "Inconnu",
        ["itIT"] = "Sconosciuto",
        ["koKR"] = "알 수 없음",
        ["ptBR"] = "Desconhecido",
        ["ruRU"] = "Неизвестно",
        ["zhCN"] = "未知",
        ["zhTW"] = "未知",
    },
    ["Automatically blacklisted"] = {
        ["enUS"] = true,
        ["deDE"] = "Automatisch ausgeschlossen",
        ["esES"] = "Incluido automáticamente en la lista negra",
        ["esMX"] = "Incluido automáticamente en la lista negra",
        ["frFR"] = "Mis automatiquement sur liste noire",
        ["itIT"] = "Aggiunto automaticamente alla lista nera",
        ["koKR"] = "자동 차단됨",
        ["ptBR"] = "Adicionado automaticamente à lista negra",
        ["ruRU"] = "Автоматически в черном списке",
        ["zhCN"] = "已自动加入黑名单",
        ["zhTW"] = "已自動加入黑名單",
    },
    ["Manually blacklisted"] = {
        ["enUS"] = true,
        ["deDE"] = "Manuell ausgeschlossen",
        ["esES"] = "Incluido manualmente en la lista negra",
        ["esMX"] = "Incluido manualmente en la lista negra",
        ["frFR"] = "Mis manuellement sur liste noire",
        ["itIT"] = "Aggiunto manualmente alla lista nera",
        ["koKR"] = "수동 차단됨",
        ["ptBR"] = "Adicionado manualmente à lista negra",
        ["ruRU"] = "Вручную в черном списке",
        ["zhCN"] = "已手动加入黑名单",
        ["zhTW"] = "已手動加入黑名單",
    },
    ["Parent active"] = {
        ["enUS"] = true,
        ["deDE"] = "Übergeordnete Quest aktiv",
        ["esES"] = "Misión padre activa",
        ["esMX"] = "Misión padre activa",
        ["frFR"] = "Quête parente active",
        ["itIT"] = "Missione padre attiva",
        ["koKR"] = "상위 퀘스트 활성화",
        ["ptBR"] = "Missão pai ativa",
        ["ruRU"] = "Родительская задача активна",
        ["zhCN"] = "父任务已激活",
        ["zhTW"] = "父任務已啟用",
    },
    ["Race requirement"] = {
        ["enUS"] = true,
        ["deDE"] = "Völkeranforderung",
        ["esES"] = "Requisito de raza",
        ["esMX"] = "Requisito de raza",
        ["frFR"] = "Condition de race",
        ["itIT"] = "Requisito di razza",
        ["koKR"] = "종족 조건",
        ["ptBR"] = "Requisito de raça",
        ["ruRU"] = "Требование расы",
        ["zhCN"] = "种族要求",
        ["zhTW"] = "種族要求",
    },
    ["Faction requirement"] = {
        ["enUS"] = true,
        ["deDE"] = "Fraktionsanforderung",
        ["esES"] = "Requisito de facción",
        ["esMX"] = "Requisito de facción",
        ["frFR"] = "Condition de faction",
        ["itIT"] = "Requisito di fazione",
        ["koKR"] = "진영 조건",
        ["ptBR"] = "Requisito de facção",
        ["ruRU"] = "Требование фракции",
        ["zhCN"] = "阵营要求",
        ["zhTW"] = "陣營要求",
    },
    ["Incomplete pre-quest"] = {
        ["enUS"] = true,
        ["deDE"] = "Prequest nicht abgeschlossen",
        ["esES"] = "Falta misión requisita",
        ["esMX"] = "Falta misión requisita",
        ["frFR"] = "Pré-quête incomplète",
        ["itIT"] = "Pre-quest incompleta",
        ["koKR"] = "선행 퀘스트 미수행",
        ["ptBR"] = "Faltando missão requisita",
        ["ruRU"] = "Не выполнено предзадание",
        ["zhCN"] = "前置任务未完成",
        ["zhTW"] = "需要前置任務未完成",
    },
    ["Class requirement"] = {
        ["enUS"] = true,
        ["deDE"] = "Klassenanforderung",
        ["esES"] = "Requisito de clase",
        ["esMX"] = "Requisito de clase",
        ["frFR"] = "Condition de classe",
        ["itIT"] = "Requisito di classe",
        ["koKR"] = "직업 조건",
        ["ptBR"] = "Requisito de classe",
        ["ruRU"] = "Требование класса",
        ["zhCN"] = "职业要求",
        ["zhTW"] = "職業要求",
    },
    ["Reputation requirement"] = {
        ["enUS"] = true,
        ["deDE"] = "Rufanforderung",
        ["esES"] = "Requisito de reputación",
        ["esMX"] = "Requisito de reputación",
        ["frFR"] = "Condition de réputation",
        ["itIT"] = "Requisito di reputazione",
        ["koKR"] = "평판 조건",
        ["ptBR"] = "Requisito de reputação",
        ["ruRU"] = "Требование репутации",
        ["zhCN"] = "声望要求",
        ["zhTW"] = "聲望要求",
    },
    ["Profession requirement"] = {
        ["enUS"] = true,
        ["deDE"] = "Berufsanfor­derung",
        ["esES"] = "Requisito de profesión",
        ["esMX"] = "Requisito de profesión",
        ["frFR"] = "Condition de métier",
        ["itIT"] = "Requisito di professione",
        ["koKR"] = "전문 기술 조건",
        ["ptBR"] = "Requisito de profissão",
        ["ruRU"] = "Требование профессии",
        ["zhCN"] = "专业要求",
        ["zhTW"] = "專業要求",
    },
    ["Incomplete pre-quest group"] = {
        ["enUS"] = true,
        ["deDE"] = "Prequests nicht abgeschlossen",
        ["esES"] = "Faltan misiones requisitas",
        ["esMX"] = "Faltan misiones requisitas",
        ["frFR"] = "Pré-quêtes incomplètes",
        ["itIT"] = "Pre-quest incomplete",
        ["koKR"] = "선행 퀘스트 미수행 그룹",
        ["ptBR"] = "Faltando missões requisitas",
        ["ruRU"] = "Не выполнены предзадания",
        ["zhCN"] = "前置任务组未完成",
        ["zhTW"] = "需要前置任務未完成群",
    },
    ["Inactive parent"] = {
        ["enUS"] = true,
        ["deDE"] = "Übergeordnete Quest inaktiv",
        ["esES"] = "Misión padre inactiva",
        ["esMX"] = "Misión padre inactiva",
        ["frFR"] = "Quête parente inactive",
        ["itIT"] = "Missione padre inattiva",
        ["koKR"] = "상위 퀘스트 비활성화",
        ["ptBR"] = "Missão pai inativa",
        ["ruRU"] = "Родительская задача неактивна",
        ["zhCN"] = "父任务未激活",
        ["zhTW"] = "父任務未啟用",
    },
    ["Later quest completed or active"] = {
        ["enUS"] = true,
        ["deDE"] = "Spätere Quest abgeschlossen oder aktiv",
        ["esES"] = "Misión posterior completada o activa",
        ["esMX"] = "Misión posterior completada o activa",
        ["frFR"] = "Quête ultérieure terminée ou active",
        ["itIT"] = "Missione successiva completata o attiva",
        ["koKR"] = "이후 퀘스트 완료 또는 활성",
        ["ptBR"] = "Missão posterior concluída ou ativa",
        ["ruRU"] = "Позднее задание выполнено или активно",
        ["zhCN"] = "后续任务已完成或激活",
        ["zhTW"] = "後續任務已完成或啟用",
    },
    ["Exclusive quest completed"] = {
        ["enUS"] = true,
        ["deDE"] = "Exklusive Quest abgeschlossen",
        ["esES"] = "Misión exclusiva completada",
        ["esMX"] = "Misión exclusiva completada",
        ["frFR"] = "Quête exclusive terminée",
        ["itIT"] = "Missione esclusiva completata",
        ["koKR"] = "독점 퀘스트 완료",
        ["ptBR"] = "Missão exclusiva concluída",
        ["ruRU"] = "Эксклюзивное задание выполнено",
        ["zhCN"] = "独占任务已完成",
        ["zhTW"] = "專屬任務已完成",
    },
    ["Exclusive quest in quest log"] = {
        ["enUS"] = true,
        ["deDE"] = "Exklusive Quest im Questlog",
        ["esES"] = "Misión exclusiva en el registro de misiones",
        ["esMX"] = "Misión exclusiva en el registro de misiones",
        ["frFR"] = "Quête exclusive dans le journal de quêtes",
        ["itIT"] = "Missione esclusiva nel registro missioni",
        ["koKR"] = "퀘스트 목록의 독점 퀘스트",
        ["ptBR"] = "Missão exclusiva no registro de missões",
        ["ruRU"] = "Эксклюзивное задание в журнале заданий",
        ["zhCN"] = "任务日志中的独占任务",
        ["zhTW"] = "任務日誌中的專屬任務",
    },
    ["Inactive daily"] = {
        ["enUS"] = true,
        ["deDE"] = "Inaktive Tagesquest",
        ["esES"] = "Diario inactivo",
        ["esMX"] = "Diario inactivo",
        ["frFR"] = "Quête journalière inactive",
        ["itIT"] = "Missione giornaliera inattiva",
        ["koKR"] = "일일 퀘스트 비활성화",
        ["ptBR"] = "Diário inativo",
        ["ruRU"] = "Неактивная ежедневная задача",
        ["zhCN"] = "日常任务未激活",
        ["zhTW"] = "日常任務未啟用",
    },
    ["Profession specialization requirement"] = {
        ["enUS"] = true,
        ["deDE"] = "Berufsspezialisierungsanforderung",
        ["esES"] = "Requisito de especialización de profesión",
        ["esMX"] = "Requisito de especialización de profesión",
        ["frFR"] = "Condition de spécialisation de métier",
        ["itIT"] = "Requisito di specializzazione della professione",
        ["koKR"] = "전문 기술 전문화 조건",
        ["ptBR"] = "Requisito de especialização de profissão",
        ["ruRU"] = "Требование специализации профессии",
        ["zhCN"] = "专业专精要求",
        ["zhTW"] = "專業專精要求",
    },
    ["Spell not yet learned"] = {
        ["enUS"] = true,
        ["deDE"] = "Zauber noch nicht erlernt",
        ["esES"] = "Hechizo aún no aprendido",
        ["esMX"] = "Hechizo aún no aprendido",
        ["frFR"] = "Sort non encore appris",
        ["itIT"] = "Incantesimo non ancora appreso",
        ["koKR"] = "아직 습득하지 않은 주문",
        ["ptBR"] = "Feitiço ainda não aprendido",
        ["ruRU"] = "Заклинание ещё не изучено",
        ["zhCN"] = "尚未学习该法术",
        ["zhTW"] = "尚未學習該法術",
    },
    ["Already learned spell"] = {
        ["enUS"] = true,
        ["deDE"] = "Zauber bereits erlernt",
        ["esES"] = "Hechizo ya aprendido",
        ["esMX"] = "Hechizo ya aprendido",
        ["frFR"] = "Sort déjà appris",
        ["itIT"] = "Incantesimo già appreso",
        ["koKR"] = "이미 습득한 주문",
        ["ptBR"] = "Feitiço já aprendido",
        ["ruRU"] = "Заклинание уже изучено",
        ["zhCN"] = "已学习该法术",
        ["zhTW"] = "已學習該法術",
    },
    ["Achievement requirement"] = {
        ["enUS"] = true,
        ["deDE"] = "Erfolgsanforderung",
        ["esES"] = "Requisito de logro",
        ["esMX"] = "Requisito de logro",
        ["frFR"] = "Condition de haut fait",
        ["itIT"] = "Requisito di successo",
        ["koKR"] = "업적 조건",
        ["ptBR"] = "Requisito de conquista",
        ["ruRU"] = "Требование достижения",
        ["zhCN"] = "成就要求",
        ["zhTW"] = "成就要求",
    },
    ["Another breadcrumb is active"] = {
        ["enUS"] = true,
        ["deDE"] = "Eine andere Brotkrumen-Quest ist aktiv",
        ["esES"] = "Otra misión de rastro está activa",
        ["esMX"] = "Otra misión de migajas está activa",
        ["frFR"] = "Une autre quête préliminaire est active",
        ["itIT"] = "Un'altra missione di orientamento è attiva",
        ["koKR"] = "다른 길잡이가 활성화됨",
        ["ptBR"] = "Outra missão de encaminhamento está ativa",
        ["ruRU"] = "Другое предзадание активно",
        ["zhCN"] = "另一个任务线索已激活",
        ["zhTW"] = "另一個任務線索已啟用",
    },
    ["A breadcrumb is active"] = {
        ["enUS"] = true,
        ["deDE"] = "Eine Brotkrumen-Quest ist aktiv",
        ["esES"] = "Una misión de rastro está activa",
        ["esMX"] = "Una misión de migajas está activa",
        ["frFR"] = "Une quête préliminaire est active",
        ["itIT"] = "Una missione di orientamento è attiva",
        ["koKR"] = "길잡이가 활성화됨",
        ["ptBR"] = "Uma missão de encaminhamento está ativa",
        ["ruRU"] = "Предзадание активно",
        ["zhCN"] = "任务线索已激活",
        ["zhTW"] = "任務線索已啟用",
    },
    ["Daily quest not active"] = {
        ["enUS"] = true,
        ["deDE"] = "Tagesquest nicht aktiv",
        ["esES"] = "Misión diaria no activa",
        ["esMX"] = "Misión diaria no activa",
        ["frFR"] = "Quête journalière non active",
        ["itIT"] = "Missione giornaliera non attiva",
        ["koKR"] = "일일 퀘스트 비활성",
        ["ptBR"] = "Missão diária não ativa",
        ["ruRU"] = "Ежедневное задание неактивно",
        ["zhCN"] = "日常任务未激活",
        ["zhTW"] = "日常任務未啟用",
    },
    ["Level too low"] = {
        ["enUS"] = true,
        ["deDE"] = "Stufe zu niedrig",
        ["esES"] = "Nivel demasiado bajo",
        ["esMX"] = "Nivel demasiado bajo",
        ["frFR"] = "Niveau trop bas",
        ["itIT"] = "Livello troppo basso",
        ["koKR"] = "레벨이 너무 낮음",
        ["ptBR"] = "Nível muito baixo",
        ["ruRU"] = "Слишком низкий уровень",
        ["zhCN"] = "等级过低",
        ["zhTW"] = "等級過低",
    },
    ["Level too high"] = {
        ["enUS"] = true,
        ["deDE"] = "Stufe zu hoch",
        ["esES"] = "Nivel demasiado alto",
        ["esMX"] = "Nivel demasiado alto",
        ["frFR"] = "Niveau trop élevé",
        ["itIT"] = "Livello troppo alto",
        ["koKR"] = "레벨이 너무 높음",
        ["ptBR"] = "Nível muito alto",
        ["ruRU"] = "Слишком высокий уровень",
        ["zhCN"] = "等级过高",
        ["zhTW"] = "等級過高",
    },
    ["Follow up quest active or completed"] = {
        ["enUS"] = true,
        ["deDE"] = "Folge-Quest aktiv oder abgeschlossen",
        ["esES"] = "La siguiente misión está activa o completada",
        ["esMX"] = "La siguiente misión está activa o completada",
        ["frFR"] = "La quête suivante est active ou terminée",
        ["itIT"] = "La missione successiva è attiva o completata",
        ["koKR"] = "후속 퀘스트 진행 중 또는 완료됨",
        ["ptBR"] = "A próxima missão está ativa ou concluída",
        ["ruRU"] = "Следующее задание активно или выполнено",
        ["zhCN"] = "后续任务处于进行中或已完成",
        ["zhTW"] = "後續任務處於進行中或已完成",
    },
    ["Disabling quest already turned in"] = {
        ["enUS"] = true,
        ["deDE"] = "Deaktivierende Quest ist bereits abgegeben",
        ["esES"] = "La misión desactivadora ya fue entregada",
        ["esMX"] = "La misión desactivadora ya fue entregada",
        ["frFR"] = "La quête désactivatrice a déjà été rendue",
        ["itIT"] = "La missione disattivante è già stata consegnata",
        ["koKR"] = "이미 완료한 퀘스트 비활성화",
        ["ptBR"] = "A missão desativadora já foi entregue",
        ["ruRU"] = "Отключение уже сданного задания",
        ["zhCN"] = "禁用已提交的任务",
        ["zhTW"] = "禁用已提交的任務",
    },
    ["Enabling quest not active nor turned in"] = {
        ["enUS"] = true,
        ["deDE"] = "Aktivierende Quest ist nicht im Questlog und nicht abgegeben",
        ["esES"] = "Activando misión no activa ni entregada",
        ["esMX"] = "Activando misión no activa ni entregada",
        ["frFR"] = "Active la quête ni active ni rendue",
        ["itIT"] = "La missione abilitante non è attiva né consegnata",
        ["koKR"] = "활성화되지 않았거나 완료하지 않은 퀘스트 활성화",
        ["ptBR"] = "Ativando missão não ativa nem entregue",
        ["ruRU"] = "Включение неактивного и несданного задания",
        ["zhCN"] = "启用既未激活也未提交的任务",
        ["zhTW"] = "啟用既未啟動也未提交的任務",
    },
    ["Unavailable"] = {
        ["enUS"] = true,
        ["deDE"] = "Nicht verfügbar",
        ["esES"] = "No disponible",
        ["esMX"] = "No disponible",
        ["frFR"] = "Indisponible",
        ["itIT"] = "Non disponibile",
        ["koKR"] = "사용 불가",
        ["ptBR"] = "Indisponível",
        ["ruRU"] = "Недоступно",
        ["zhCN"] = "无法完成",
        ["zhTW"] = "無法完成",
    },
    ["Event inactive"] = {
        ["enUS"] = true,
        ["deDE"] = "Event inaktiv",
        ["esES"] = "Evento inactivo",
        ["esMX"] = "Evento inactivo",
        ["frFR"] = "Événement inactif",
        ["itIT"] = "Evento inattivo",
        ["koKR"] = "이벤트 비활성",
        ["ptBR"] = "Evento inativo",
        ["ruRU"] = "Событие неактивно",
        ["zhCN"] = "节日未开启",
        ["zhTW"] = "節慶未開啟",
    },
    ["Reputation too low"] = {
        ["enUS"] = true,
        ["deDE"] = "Ruf zu gering",
        ["esES"] = "Reputación demasiado baja",
        ["esMX"] = "Reputación demasiado baja",
        ["frFR"] = "Réputation trop basse",
        ["itIT"] = "Reputazione troppo bassa",
        ["koKR"] = "평판이 너무 낮습니다",
        ["ptBR"] = "Reputação muito baixa",
        ["ruRU"] = "Репутация слишком низкая",
        ["zhCN"] = "声望太低",
        ["zhTW"] = "聲望太低",
    },
    ["Reputation too high"] = {
        ["enUS"] = true,
        ["deDE"] = "Ruf zu hoch",
        ["esES"] = "Reputación demasiado alta",
        ["esMX"] = "Reputación demasiado alta",
        ["frFR"] = "Réputation trop élevée",
        ["itIT"] = "Reputazione troppo alta",
        ["koKR"] = "평판이 너무 높습니다",
        ["ptBR"] = "Reputação muito alta",
        ["ruRU"] = "Репутация слишком высокая",
        ["zhCN"] = "声望太高",
        ["zhTW"] = "聲望太高",
    },
    ["Profession missing"] = {
        ["enUS"] = true,
        ["deDE"] = "Beruf fehlt",
        ["esES"] = "Profesión faltante",
        ["esMX"] = "Profesión faltante",
        ["frFR"] = "Métier manquante",
        ["itIT"] = "Professione mancante",
        ["koKR"] = "전문화를 찾을 수 없습니다",
        ["ptBR"] = "Profissão ausente",
        ["ruRU"] = "Профессия отсутствует",
        ["zhCN"] = "缺少专业",
        ["zhTW"] = "缺少專業技能",
    },
    ["Profession skill"] = {
        ["enUS"] = true,
        ["deDE"] = "Beruffähigkeit",
        ["esES"] = "Habilidad de profesión",
        ["esMX"] = "Habilidad de profesión",
        ["frFR"] = "Compétence de métier",
        ["itIT"] = "Abilità di professione",
        ["koKR"] = "전문화 스킬",
        ["ptBR"] = "Habilidade de profissão",
        ["ruRU"] = "Навык профессии",
        ["zhCN"] = "专业技能",
        ["zhTW"] = "專業技能",
    },
    ["Profession rank"] = {
        ["enUS"] = true,
        ["deDE"] = "Berufrang",
        ["esES"] = "Rango de profesión",
        ["esMX"] = "Rango de profesión",
        ["frFR"] = "Rang de métier",
        ["itIT"] = "Grado della professione",
        ["koKR"] = "전문화 랭크",
        ["ptBR"] = "Grau da profissão",
        ["ruRU"] = "Уровень профессии",
        ["zhCN"] = "专业级别",
        ["zhTW"] = "專業等級",
    },
    ["Import"] = {
        ["enUS"] = true,
        ["deDE"] = "Importieren",
        ["esES"] = "Importar",
        ["esMX"] = "Importar",
        ["frFR"] = "Importer",
        ["itIT"] = "Importare",
        ["koKR"] = "가져오기",
        ["ptBR"] = "Importar",
        ["ruRU"] = "Импорт",
        ["zhCN"] = "导入",
        ["zhTW"] = "匯入",
    },
    ["Import Journey data"] = {
        ["enUS"] = true,
        ["deDE"] = "Reisedaten importieren",
        ["esES"] = "Importar datos del viaje",
        ["esMX"] = "Importar datos del viaje",
        ["frFR"] = "Importer les données du voyage",
        ["itIT"] = "Importare i dati del viaggio",
        ["koKR"] = "여정 데이터 가져오기",
        ["ptBR"] = "Importar dados da viagem",
        ["ruRU"] = "Импортировать данные Путешествия",
        ["zhCN"] = "导入旅程数据",
        ["zhTW"] = "匯入旅程資料",
    },
    ["Browse other characters on this account and import their journey data."] = {
        ["enUS"] = true,
        ["deDE"] = "Finde andere Charakter auf diesem Account, um deren Reisedaten zu importieren.",
        ["esES"] = "Busca otros personajes de esta cuenta e importa sus datos del viaje.",
        ["esMX"] = "Busca otros personajes de esta cuenta e importa sus datos del viaje.",
        ["frFR"] = "Recherche les autres personnages de ce compte et importez leurs données de voyage.",
        ["itIT"] = "Cerca altri personaggi su questo account e importa i loro dati del viaggio.",
        ["koKR"] = "이 계정의 다른 캐릭터를 찾아 그 여정 데이터를 가져옵니다.",
        ["ptBR"] = "Procura outros personagens desta conta e importe os dados da viagem deles.",
        ["ruRU"] = "Посмотреть других персонажей этого аккаунта и импортировать данные их путешествий.",
        ["zhCN"] = "浏览此账号上的其他角色并导入他们的旅程数据",
        ["zhTW"] = "查看此帳號上的其他角色並匯入他們的旅程資料",
    },
    ["Select a character to import"] = {
        ["enUS"] = true,
        ["deDE"] = "Wähle einen Charakter zum Importieren",
        ["esES"] = "Selecciona un personaje para importar",
        ["esMX"] = "Selecciona un personaje para importar",
        ["frFR"] = "Sélectionnez un personnage à importer",
        ["itIT"] = "Seleziona un personaggio da importare",
        ["koKR"] = "가져올 캐릭터 선택",
        ["ptBR"] = "Seleciona um personagem para importar",
        ["ruRU"] = "Выберите персонажа для импорта",
        ["zhCN"] = "选择角色以导入",
        ["zhTW"] = "選擇要匯入的角色",
    },
    ["No other characters with journey data found."] = {
        ["enUS"] = true,
        ["deDE"] = "Es wurde kein anderer Charakter mit Reisedaten gefunden.",
        ["esES"] = "No se encontraron otros personajes con datos del viaje.",
        ["esMX"] = "No se encontraron otros personajes con datos del viaje.",
        ["frFR"] = "Aucun autre personnage avec des données de voyage trouvé.",
        ["itIT"] = "Nessun altro personaggio con dati del viaggio trovato.",
        ["koKR"] = "여정 데이터가 있는 다른 캐릭터를 찾을 수 없습니다.",
        ["ptBR"] = "Nenhum outro personagem com dados da viagem foi encontrado.",
        ["ruRU"] = "Не найдено персонажей с путешествиями.",
        ["zhCN"] = "未找到其他角色的旅程数据",
        ["zhTW"] = "未找到其它有旅程資料的角色",
    },
    ["Journey imported. Open the My Journey tab to see the changes."] = {
        ["enUS"] = true,
        ["deDE"] = "Reisedaten importiert. Öffne \"Meine Reise\", um die Änderungen zu sehen.",
        ["esES"] = "Viaje importado. Abre la pestaña Mi viaje para ver los cambios.",
        ["esMX"] = "Viaje importado. Abre la pestaña Mi viaje para ver los cambios.",
        ["frFR"] = "Voyage importé. Ouvrez l’onglet Mon voyage pour voir les modifications.",
        ["itIT"] = "Viaggio importato. Apri la scheda Il mio viaggio per vedere le modifiche.",
        ["koKR"] = "여정이 가져와졌습니다. '내 여정' 탭에서 변경사항을 확인하세요.",
        ["ptBR"] = "Viagem importada. Abra a aba Minha viagem para ver as alterações.",
        ["ruRU"] = "Путешествие импортировано. Откройте вкладку \"Путешествие\", чтобы увидеть изменения.",
        ["zhCN"] = "已导入旅程，打开我的旅程页面查看这些变化",
        ["zhTW"] = "旅程匯入完成，請打開「我的旅程」頁籤查看變更",
    },
    ["Import this journey data? This will overwrite your current journey."] = {
        ["enUS"] = true,
        ["deDE"] = "Diese Reisedaten importieren? Die aktuellen Daten werden dadurch überschrieben.",
        ["esES"] = "¿Importar estos datos del viaje? Esto sobrescribirá tu viaje actual.",
        ["esMX"] = "¿Importar estos datos del viaje? Esto sobrescribirá tu viaje actual.",
        ["frFR"] = "Importer ces données de voyage ? Cela écrasera votre voyage actuel.",
        ["itIT"] = "Importare questi dati del viaggio? Questo sovrascriverà il tuo viaggio attuale.",
        ["koKR"] = "이 여정 데이터를 가져오시겠습니까? 현재 여정을 덮어씁니다.",
        ["ptBR"] = "Importar estes dados da viagem? Isso substituirá sua viagem atual.",
        ["ruRU"] = "Импортировать данные этого путешествия? Это перезапишет текущее путешествие.",
        ["zhCN"] = "要导入此旅程数据吗？这会覆盖你当前的旅程",
        ["zhTW"] = "要匯入這份旅程資料嗎？這將會覆蓋你目前的旅程",
    },
    ["Disabling quest is active"] = {
        ["enUS"] = true,
        ["deDE"] = "Deaktivierende Quest ist akttiv",
        ["esES"] = "La misión desactivadora está activa",
        ["esMX"] = "La misión desactivadora está activa",
        ["frFR"] = "La quête désactivatrice est active",
        ["itIT"] = "La missione disattivante è attiva",
        ["koKR"] = "비활성화 퀘스트가 활성화되었습니다",
        ["ptBR"] = "A missão desativadora está ativa",
        ["ruRU"] = "Отключение активного задания",
        ["zhCN"] = "禁用任务已激活",
        ["zhTW"] = "停用任務已啟用",
    },
    ["Missing Arena Rating"] = {
        ["enUS"] = true,
        ["deDE"] = "Fehlende Arenawertung",
        ["esES"] = "Índice de arena faltante",
        ["esMX"] = "Índice de arena faltante",
        ["frFR"] = "Cote d'arène manquante",
        ["itIT"] = "Valutazione dell'arena mancante",
        ["koKR"] = false,
        ["ptBR"] = false,
        ["ruRU"] = "Недостаточный рейтинг арены",
        ["zhCN"] = "缺少竞技场等级",
        ["zhTW"] = false,
    },
}

for k, v in pairs(myJourneyLocales) do
    l10n.translations[k] = v
end
