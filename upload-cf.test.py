"""Offline uploader tests using temporary Git remotes and a fake curl. Run: python3 upload-cf.test.py."""
...
SCRIPT = Path(__file__).with_name("upload-cf.sh").resolve()
...
class CurseForgeUploadTests(unittest.TestCase):
    def setUp(self):
        ...
        self.marker = "bundle/curse/v12.0.0+v1.0.0"
        ...
        self.env = dict(os.environ, PATH=str(self.bin) + os.pathsep + os.environ["PATH"],
                        CF_API_TOKEN="test-token", UPLOAD_LOG=str(self.calls), EXPECTED_MARKER=self.marker,
                        UPLOAD_STATUS="200", UPLOAD_EXIT="0", REAL_GIT=GIT, REMOTE_PATH=str(self.remote),
    ...
    def test_beta_suffix_must_match_the_tag_commit_prefix(self):
        for suffix in ("0000000", self.commit[:8]):
            with self.subTest(suffix=suffix):
                ...
                self.marker = f"bundle/curse/v12.0.0-pre.{suffix}+v1.0.0"
