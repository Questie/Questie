...
function QuestieTracker:updateTrackingFrameSize()
    if (QuestieConfig.trackerBackground == true) then
        ...
        if totalWidth < 144 then totalWidth = 144 end
...
function QuestieTracker:createOrGetTrackingButton(index)
    if QuestieTracker.questButtons[index] == nil then
        ...
        btn:SetScript("OnEnter", function()
            ...
            local quest = QuestieTrackedQuests[questHash]
        ...
        if ( IsShiftKeyDown() ) then
            ...
            elseif ( WIM_EditBoxInFocus ) then
                ...
                if ( IsQuestWatched(questIndex) ) then
                    ...
                    --[[--------------------------------------------------------------------
                    -- We want to be able to track anything in the Defalut WoW Quest Tracker
                    ------------------------------------------------------------------------
...
function QuestieTracker:syncQuestLog()
    ...
    while qc < nQuests do
        ...
        if not isHeader and not isCollapsed then
            local hash = Questie:GetHashFromName(questName)
            if ( not isHeader and QuestLogSync(id) and not QuestieTracker:isTracked(questName) ) then
                if QuestieTrackedQuests[hash] then
                    QuestieTrackedQuests[hash]["tracked"] = true
                    QuestieTracker:addQuestToTracker(hash, id, level)
                else
...
---------------------------------------------------------------------------------------------------
--Captures and autotracks accepted quests using the Default Blizzard QuestLog
---------------------------------------------------------------------------------------------------
            ...
            while qc < nQuests do
                ...
                if not isHeader and not isCollapsed then
                    if ( not isHeader and IsQuestWatched(id) and not QuestieTracker:isTracked(questName) ) then
                        AutoQuestWatch_Insert(id, QUEST_WATCH_NO_EXPIRE)
                        QuestWatch_Update()
