...
- Subtractive baseline: the pre-rebase whole-file deletion ended at
  `14bb2681f8a349a0470c8deb1f37c238ea72ae80` and clean mixed-runtime subtraction ended at
  `8b63c04beadef59b648cb558247609651e1f19e1`. The commit containing the finalized rebased evidence
...
- Historical validation: the incoming clean-baseline commit recorded a green local suite at its
  pre-rebase tip. That result was not rerun against the rebased stack with support cleanup
...
| TDB-06 | Adapt raw entity-table consumers | done | clean baseline | TDB-02 | Townsfolk and Available Quests use composed ID maps/queries, raw fallbacks are gone, and the missing-Item raw writer is removed. Fresh bindings and RuntimeItemRepair still supply their runtime seams. |
...
| TDB-10 | Delete dead compiler, raw data, generated lookups, and validators | done | baseline deletion | TDB-09 | Pre-rebase whole-file subtraction ended at `14bb2681f8a349a0470c8deb1f37c238ea72ae80`: 281 tracked files and 5,042,232 deleted-file lines. Clean mixed-runtime subtraction ended at `8b63c04beadef59b648cb558247609651e1f19e1`; full runtime validation still gates the combined merge after `implementation`. |
| TDB-11 | Read Zone, XP, Drop, and faction-template data from `Support` | not started | - | TDB-02, QuestieTDB #15 | Baseline payload deletion is complete, but provider integration is not started. Keep Questie's behavior wrappers, do not restore known-stale support copies, and validate flavor-correct provider data before the combined merge. |
