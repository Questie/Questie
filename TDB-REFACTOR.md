...
- Subtractive baseline: historical pre-rebase whole-file deletion ended at
  ...
  `cf4349e9f647f3c1b077421863fc53ef6031da44`. The rebased baseline also removes 24 support payloads
  and their 51 flavor TOC entries while retaining the consumer wrappers. Tests use focused inline
  zone fixtures instead of production payloads
- Implementation: branch `QuestieTDB-implementation`, created from the historical baseline handoff
  commit `ad8e6ec19979f42eeecc1e3262575014fa8c3fe8`, adds the Contract Version 1 test double, the
  ...
  consumer verification. TDB-11 provider support binding is not part of this replay point; it lands
  later in the historical replay. Issue #15 parity validation remains pending
- Historical validation: pre-rebase implementation validation recorded 1,530 Busted successes,
  clean production luacheck across 329 files, passing loader-usage and diff checks, and empty
  retirement greps. These checks were not rerun against the current rebased stack
- Distribution and release packaging: deferred

The implementation started fresh from the recorded subtractive baseline, Contract Version 1,
retained QuestiePolicy producers, and the authoritative handovers. Historical code and tests could
clarify behavior, but did not anchor architecture or restore compatibility scaffolding.
...
| TDB-10 | Delete dead compiler, raw data, generated lookups, and validators | done | baseline deletion | TDB-09 | Historical pre-rebase whole-file subtraction ended at `14bb2681f8a349a0470c8deb1f37c238ea72ae80`: 281 tracked files and 5,042,232 deleted-file lines. Clean mixed-runtime subtraction ended at `8b63c04beadef59b648cb558247609651e1f19e1`, with reviewed clean-baseline fixes ending at `cf4349e9f647f3c1b077421863fc53ef6031da44`; full runtime validation still gates the combined merge after `implementation`. |
| TDB-11 | Read Zone, XP, Drop, and faction-template data from `Support` | not started | - | TDB-02, QuestieTDB #15 | Baseline deletion of 24 payloads and 51 TOC entries is complete, but provider integration is not started. Keep Questie's behavior wrappers, do not restore known-stale support copies, and validate flavor-correct provider data before the combined merge. |
| TDB-12 | Replace database validation CI with a pinned integration check | blocked | QuestieTDB-implementation | TDB-10, TDB-11, QuestieTDB #19 | The old `db-validation` matrix is removed and loader-usage validation remains in the unit-test job. The provider repository documents no consumer-side integration command and pins Questie through its own `QUESTIE_COMMIT`; the pinned Database Integration Check waits for the final provider revision and command. |
...
- Subtractive baseline completion: WP-00 extracted the retained Classic/TBC Questie policy producers at `a85d6c5a2ad1e77f431907ef70d4163f623c1bd1`; push-triggered CI run 33496726477 passed for that exact SHA, and evidence was recorded at `09e0178e79775782cdabd75f506dccd6e8ec0698`. Six deletion commits `99493b08` through `14bb2681f8a349a0470c8deb1f37c238ea72ae80`, measured by `09e0178e..14bb2681f`, removed 281 tracked files and 5,042,232 deleted-file lines while retaining QuestiePolicy, Titan quest tags, blacklists, event/content-phase state, UI and Zone/Category localization, QuestieStream, and support consumer wrappers. The rebased stack also removes 24 support payloads and 51 TOC entries while keeping those wrappers and using focused inline zone fixtures. Nine pre-rebase cleanup commits `0b02060c` through `8b63c04beadef59b648cb558247609651e1f19e1` removed mixed-runtime compiler/entity residue, converted raw consumers, retained semantic constants, added Migration 38, and restored a focused provider metadata test seam. Review fixes at `cf4349e9f647f3c1b077421863fc53ef6031da44` restored NPC flag semantics, bound QuestiePolicy tests to provider metadata, and kept WP-06 verification open. Objective Order was not extracted. That reviewed tip recorded 1,424 Busted successes, clean production luacheck across 322 files, passing loader-usage and diff checks, and empty production retirement greps. These historical checks were not rerun for the current rebased stack.
- Implementation completion: `QuestieTDB-implementation` started at historical baseline handoff commit
  `ad8e6ec19979f42eeecc1e3262575014fa8c3fe8` and landed, in separately reviewable commits, the
  ...
  registration/provider index split, and Townsfolk and Available Quests verification. Pre-rebase
  validation recorded 1,530 Busted successes, clean luacheck across 329 files, passing loader-usage
  and `git diff --check`, and empty retirement greps. These results are historical and were not rerun
  for the current rebased stack. TDB-11 support binding is not part of this replay point and lands
  later in the historical replay. TDB-12 stays blocked because the provider documents no
  consumer-side pinned integration command.
- Historical implementation review: two reviewers (lifecycle, ownership, and Contract assumptions;
  test value and mock fidelity) found two blockers the Contract mock had hidden. The provider reads
  `0`, never nil, for an absent number field of an existing entity, which made
